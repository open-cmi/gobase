...
func (l *TimeWriter) Write(p []byte) (n int, err error) {
	...
	// 优化：仅在日期字符串变化时才尝试 rotate
	now := time.Now()
	if l.curDateStr != now.Format(dateFormat) {
		l.rotate()
	}
...
func (l *TimeWriter) oldLogFiles() ([]logInfo, error) {
	...
	for _, f := range files {
		...
		name := f.Name()
		if name == filepath.Base(l.curFilename) {
			continue
		}

		// os.ReadDir 返回的是 os.DirEntry，需要通过 Info() 获取 os.FileInfo
		info, err := f.Info()
		if err != nil {
			continue // 无法获取文件信息则跳过
		}

		if t, err := l.timeFromName(name, prefix, ext); err == nil {
			logFiles = append(logFiles, logInfo{timestamp: t, FileInfo: info})
			continue
		}
...
func (l *TimeWriter) openNew() error {
	...
	name := l.filenameForDate(dateStr)
...
func (l *TimeWriter) openExistingOrNew(writeLen int) error {
	now := time.Now()
	dateStr := now.Format(dateFormat)
	filename := l.filenameForDate(dateStr)
...
// 抽离出 filename 逻辑，避免副作用
func (l *TimeWriter) filename() string {
	return l.filenameForDate(time.Now().Format(dateFormat))
}

func (l *TimeWriter) filenameForDate(date string) string {
	prefix := l.LogFilePrefix
	if prefix == "" {
		prefix = filepath.Base(os.Args[0])
	}
	name := fmt.Sprintf("%s.%s.log", filepath.Base(prefix), date)
	if l.Dir != "" {
